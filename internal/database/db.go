...
type Repository interface {
    FindTranslation(ctx context.Context, keyword, context string) (*models.TranslationResponse, error)
...
func (r *GormRepository) FindTranslation(ctx context.Context, keyword, contextStr string) (*models.TranslationResponse, error) {
    var result models.TranslationResponse
    // Use .WithContext for potential cancellation/timeouts
    err := r.db.WithContext(ctx).
    Where("keyword = ? AND context = ?", keyword, contextStr).
    First(&result).Error
